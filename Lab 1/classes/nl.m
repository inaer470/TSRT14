...
if nargin<2
   error('NL nl2lss: two input arguments required')
elseif ~isa(m,'nl')
   error('NL nl2lss: first input argument must be an NL object')
elseif ~isa(z,'sig')
   error('NL nl2lss: second input argument must be a SIG object')
elseif size(z,1)>1
   error('NL nl2lss: SIG object z has to be one sample, not a signal')
elseif  isempty(z.x)
   error('NL nl2lss: SIG object z must contain a state z.x')
end
...
if size(z.x,2)~=nx
   error('NL nl2lss: SIG object state z.x does not match the state dimension in m')
elseif nu>0 & isempty(z.u)
   error('NL nl2lss: model m has an input, but the SIG object z.u is empty')
elseif nu~=size(z.u)
   error('NL nl2lss: model m input has not the same dimension as the SIG object z.u')
end
