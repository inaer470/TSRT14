...
%     y0 = [];
%     for l = 2:size(tphat, 2)
%         y0 = [y0 tphat(k, 1) - tphat(k,l)];
%     end
% yy(k,:) = y0;
% end
% y0 = yy;
...
yy = zeros(length(tphat),max(cumsum(1:size(tphat,2)-1)));
for i = 1:length(tphat)
y0 = [];
for j = 1:(size(tphat,2)-1)
    for k = j+1:size(tphat,2)
        y0 = [y0 tphat(i,j)-tphat(i,k)];
    end
end
    yy(i,:) = y0;
end
...
% 
%     % Update r0
%     sm_TDOA1.x0 = [shat.x0(1:2); min(tphat(i+1,:))-0.002];
% %    
% %     
% %     plot(shat, 'conf', 90)
% %     hold on
