...
% ORIENTATIONVIEW  Class to visualize an orientation
  ...
  properties(GetAccess='public')
    % outlier_acc  Toggled by pressing {'a', 'A'}.  When true, the
    % accelerometer outlier indicator is displayed.
    outlier_acc = true;
    % outlier_mag  Toggled by pressing {'m', 'M'}.  When true, the
    % magnetometer outlier indicator is displayed.
    outlier_mag = true;
    quit = false;  % Is set to true when {'q', 'Q'} has been pressed.
  end
  ...
  methods
    ...
    function activateKeyboardCallback(self)
    % ACTIVATEOUTLIERCONTROL Activate the keyboard callback
      set(get(self.axis_, 'Parent'), 'WindowKeyPressFcn', @self.KeyPressed);
    end
    ...
    function KeyPressed(self, obj, event)
    % KEYPRESSED Callback that is used to capture when a key has been
    % pressed
      switch event.Character
        case {'a', 'A'}
          self.outlier_acc = ~self.outlier_acc;
        case {'m', 'M'}
          self.outlier_mag = ~self.outlier_mag;
        case {'q', 'Q'}
          self.quit = true;
      end
    end
    ...
    % SETACCDIST Set acceleration disturbance indicator (external acceleration present)
      if ~self.outlier_acc
        set(self.hText_(4:5), 'Visible', 'off');
      elseif flag
    ...
    % SETMAGDIST Set magnetometer disturbance indicator
      if ~self.outlier_mag
        set(self.hText_(2:3), 'Visible', 'off');
      elseif flag
